...
function surroundByteCountStr(byteCountStr, byteCount) {
  return ("var byteC=" + byteCount + ";").concat(byteCountStr, "var wBuffer=bag.allocUnsafe(byteC);");
}
...
function getCompiledSchema(schema, schemaIsArray) {
  ...
  if (typeof schema === "string") { // Single item schema
    ...
    strEncodeFunction = "bag.byteOffset=0; var wBuffer=bag.allocUnsafe(" + dtBytes + ");" + getWriteTypeDictStr(dt, "json") + "return wBuffer;";
...
function build(schema) { 
  ...
  return {
    ...
    "decode": function(buffer) { 
      var buf = buffer instanceof ArrayBuffer ? bufferFrom(buffer) : buffer;
